...
async function uploadToCloudinary(file) {
  ...
  // If Cloudinary credentials not set, use localStorage
  if (!CLOUDINARY_PRESET || !CLOUDINARY_CLOUD) {
    return new Promise((res, rej) => {
      const reader = new FileReader();
      reader.onload = (e) => {
        setProgress(100);
        setTimeout(() => { hideOverlay(); res(e.target.result); }, 500);
      };
      reader.onerror = () => { hideOverlay(); rej(new Error('File read error')); };
...
function removeItem(btn) {
  ...
  if (!el) {
    console.warn('removeItem: Could not find parent element');
    return;
  }
  el.remove();
...
function downloadDoc(type) {
  ...
  link.href = doc.data;
  link.download = doc.name || (type + '.pdf');
